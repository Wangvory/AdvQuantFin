...
if __name__ == '__main__':
    ...
    ob.show_book()
    # New Limit Order
    offer_order = LimitOrder(Side.SELL, 12.25, 100)
    print('Now we get a sell limit order {}'.format(offer_order))
    ...
    ob.show_book()
    # New Market Order
    offer_order = MarketOrder(Side.BUY, 13)
    print('Now we get a Buy market order {}'.format(offer_order))
    print('This removes the first two buy orders and creates a new price level on the sell side')
    ob.unprocessed_orders.put(offer_order)
    ob.process_order(ob.unprocessed_orders.get())
    ob.show_book()

    print('Example 3(No Opponent):')
    ob = OrderBook()
